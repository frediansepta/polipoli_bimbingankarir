                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover text-nowrap">
                            ...
                            <tbody>
                                ...
                                GROUP BY pasien.id, daftar_poli.status_periksa, periksa.id, pasien.alamat, pasien.no_ktp, pasien.no_hp, pasien.no_rm, periksa.tgl_periksa, dokter.nama, daftar_poli.keluhan, periksa.catatan";
                                                            ...
                                                            <table class="table table-hover text-nowrap">
                                                                ...
                                                                <tbody>
                                                                    <?php
                                                                        ...
                                                                        require 'config/koneksi.php';
                                                                        $ambilData = "SELECT 
                                                                            periksa.id AS idPeriksa, 
                                                                            periksa.tgl_periksa, 
                                                                            pasien.nama AS namaPasien, 
                                                                            dokter.nama AS namaDokter, 
                                                                            daftar_poli.keluhan, 
                                                                            periksa.catatan, 
                                                                            GROUP_CONCAT(obat.nama_obat SEPARATOR ', ') AS namaObat, 
                                                                            periksa.biaya_periksa AS hargaObat 
                                                                            FROM periksa 
                                                                            INNER JOIN daftar_poli ON periksa.id_daftar_poli = daftar_poli.id 
                                                                            INNER JOIN pasien ON daftar_poli.id_pasien = pasien.id 
                                                                            INNER JOIN detail_periksa ON periksa.id = detail_periksa.id_periksa 
                                                                            INNER JOIN obat ON detail_periksa.id_obat = obat.id 
                                                                            INNER JOIN jadwal_periksa ON daftar_poli.id_jadwal = jadwal_periksa.id 
                                                                            INNER JOIN dokter ON jadwal_periksa.id_dokter = dokter.id 
                                                                            WHERE dokter.id = '$id_dokter' AND pasien.id = '$idPasien' 
                                                                            GROUP BY periksa.id, periksa.tgl_periksa, pasien.nama, dokter.nama, daftar_poli.keluhan, periksa.catatan, periksa.biaya_periksa";
